...
- **NFS**: egress-gateway (HAProxy TCP 2049) が旧セグメントの `oc1-nfs` VM に中継する
...
| ② OS | Nix Flake + nixos-generators + sops-nix | NixOS LXC (lb-*, egress-gateway) / VM (dev-server, lm-server) | LXC は `system.autoUpgrade` による自動 Pull。VM は `nixos-rebuild switch --flake` で手動 Push |
...
| egress-gateway | 220 | server-1 | 10.20.1.30 | 192.168.5.220 | .5.0/24 への Gateway (NFS 中継 → 192.168.5.151:2049 / Ollama 中継 → 192.168.5.222:11434) |
