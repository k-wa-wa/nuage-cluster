...
- **vmbr0 — VLAN (192.168.5.0/24)**: Proxmox 管理 (GUI/SSH) と SDN の出口 (SNAT)。上流は「Internet → メインルーター (192.168.1.0/24) → Omada VLAN ルーター (192.168.1.201)」の 2 段構成
- **vmbr10 — SDN Fabric (10.0.0.0/24)**: EVPN/VXLAN アンダーレイ専用。VLAN 1/2/3 のサブインターフェースでノード間の論理 P2P リンクを構成する。nuc-1/nuc-2 は USB NIC を使用
- **vmbr11 — 予備 (10.0.1.0/24)**: 未使用の予備セグメント
- server-1 のみ `vmbr1` (192.168.1.0/24 接続) を追加で持ち、Proxy 用に使用する
- server-2 は普段は電源オフで、必要に応じて自宅 LAN に接続して稼働する (lm-server をホスト)
...
SOPS + Age による暗号化で全シークレットを Git 管理する。マスターキーは管理者ローカルの Age 鍵 1 つのみ。受信者(公開鍵)の定義はリポジトリルートの `.sops.yaml` にある。詳細は [secrets-management.md](./secrets-management.md) を参照。
