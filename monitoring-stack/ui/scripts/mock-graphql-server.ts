...
const mocks: any = {
  Query: {
    ...
    report: (parent: any, { reportId }: { reportId: string }): GraphQLTypes.Report => ({
      reportId: reportId,
  ...
  Mutation: {
    ...
    updateUserProfile: (parent: any, { displayName, notificationSettings }: { displayName?: string, notificationSettings?: GraphQLTypes.NotificationSettingInput[] }): GraphQLTypes.UserProfile => ({
      id: 'mock-user-id',
      displayName: displayName || 'Mock User',
      notificationSettings: notificationSettings ? notificationSettings.reduce((acc, setting) => ({ ...acc, [setting.key]: setting.value }), {}) : {},
    }),
...
const validateMocks = (schemaString: string, mocks: any) => {
  ...
  const checkTypeFields = (typeName: string, schemaFields: any, mockFields: any) => {
    for (const fieldName in mockFields) {
      if (!schemaFields[fieldName]) {
        console.error(`Error: Mock field '${typeName}.${fieldName}' exists but is not defined in the schema.`);
        hasError = true;
      }
    }
    for (const fieldName in schemaFields) {
      ...
      if (field.type.toString().endsWith('!') && (!mockFields || !mockFields[fieldName])) {
        console.warn(`Warning: Non-nullable field '${typeName}.${fieldName}' is not explicitly mocked.`);
      }
    }
  };
  ...
  if (queryType) {
    ...
    console.error("Error: Mock 'Query' type exists but 'Query' is not defined in the schema.");
    hasError = true;
  }
  ...
  if (mutationType) {
    ...
    console.error("Error: Mock 'Mutation' type exists but 'Mutation' is not defined in the schema.");
    hasError = true;
  }

  // Validate other object types (e.g., UserProfile, Report)
  for (const mockTypeName in mocks) {
    if (mockTypeName === 'Query' || mockTypeName === 'Mutation' || mockTypeName === 'JSONObject') {
      continue; // Already handled or scalar
    }

    const type = builtSchema.getType(mockTypeName);
    if (!type) {
      console.error(`Error: Mock type '${mockTypeName}' exists but is not defined in the schema.`);
      hasError = true;
      continue;
    }
